...
- [x] 재고 정보는 src/main/resources/products.md 파일로부터 읽어 온다.
  - [x] 상품 이름, 가격, 재고는 null이거나 빈 값일 수 없다.
  - [x] 상품의 프로모션이 null인 것은 프로모션이 없다는 의미다.
  - [ ] 상품 가격은 양수여야 한다.
- [x] 각 상품의 재고 수량을 고려하여 결제 가능 여부를 확인한다.
- [x] 고객이 상품을 구매할 때마다, 결제된 수량만큼 해당 상품의 재고에서 차감하여 수량을 관리한다.
  - 재고가 부족할 경우 `IllegalStateException`을 발생시킨다.  
- [x] 시스템은 최신 재고 상태를 유지하며, 다음 고객이 구매할 때 정확한 재고 정보를 제공한다.

### **프로모션 할인**

- [x] 프로모션 목록은 src/main/resources/promotions.md 파일로부터 읽어 온다.
- [x] 오늘 날짜가 프로모션 기간 내에 포함된 경우에만 할인을 적용한다.
- [x] 프로모션은 N개 구매 시 1개 무료 증정(Buy N Get 1 Free)의 형태로 진행된다. 1+1 또는 2+1 프로모션이 각각 지정된 상품에 적용된다.
...
1. 추가 프로모션 적용 가능 여부 확인 (프로모션 재고 확인 선행)
   - Y: get만큼 추가 구매
   - N: 프로모션 미적용 상품에 대해 정가로 구매
2. 프로모션 재고 부족시 정가 구매 여부
   - Y: 프로모션 재고 부족한 개수만큼 미적용 상품 구입
   - N: 프로모션 가능 개수만큼만 구입
3. 멤버십 할인 여부
   - Y: 프로모션 미적용 금액의 30% 할인
   - N: (X)