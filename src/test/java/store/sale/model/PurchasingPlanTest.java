...
class PurchasingPlanTest {
    ...
    @Test
    void 수량_추가_프로모션_적용() {
        Product product = createProductWithPromotion(LocalDate.parse("2024-01-01"), LocalDate.parse("2024-12-31"));
    ...
    @Test
    void 프로모션_적용_불가_금액() {
        Product product = createProductWithPromotion(LocalDate.parse("2025-01-01"), LocalDate.parse("2025-12-31"));
        PurchasingPlan plan = new PurchasingPlan(
                testDateTime(),
    ...
    private static Product createProductWithPromotion(LocalDate startDate, LocalDate endDate) {
        Product product = new Product("콜라", 1000L);
        product.setQuantity(BigInteger.TEN);
        Promotion promotion = Promotion.create("탄산2+1", BigInteger.TWO, BigInteger.ONE, startDate, endDate);
        product.setPromotionInventory(new Inventory(BigInteger.valueOf(7), promotion));
        return product;
    }
